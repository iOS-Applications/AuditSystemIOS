...
- (AFHTTPRequestOperation *)HTTPRequestOperationWithRequest:(NSURLRequest *)request
    ...
    void (^retryBlock)(AFHTTPRequestOperation *, NSError *) = ^(AFHTTPRequestOperation *operation, NSError *error) {
        ...
        if (retryCount > 0) {
            DLog(@"AutoRetry: Request failed: %@, retry begining...", error.localizedDescription);
            ...
            if (intervalInSeconds > 0) {
                DLog(@"AutoRetry: Delaying retry for %d seconds...", intervalInSeconds);
            ...
            DLog(@"AutoRetry: Request failed: %@, no more retries allowed! executing supplied failure block...", error.localizedDescription);
    ...
    AFHTTPRequestOperation *operation = [self HTTPRequestOperationWithRequest:request success:^(AFHTTPRequestOperation *operation, id responseObj) {
        DLog(@"AutoRetry: success, running success block...");
